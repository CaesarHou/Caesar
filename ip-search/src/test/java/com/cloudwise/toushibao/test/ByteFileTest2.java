...
public class ByteFileTest2 {

	public static void main(String[] args) throws Exception {
		IpInfo ipinfo = new IpInfo();
		parseLocalFile("ip_location_check2.txt", ipinfo);
		...
		try {
			...
			for(String ipLocal : ipLocals){
				byte[] iplocalBytes = ipLocal.getBytes();
		...
		try {
			...
			if (file.isFile() && file.exists()) { // 判断文件是否存在
				...
				while ((lineTxt = bufferedReader.readLine()) != null) {
					...
					String ipLocalInfo = ipStart + "|" + ipEnd + "$$$" + country + "|" + province + "|" + city + "|" + oper + "|";
...
//					TreeMap<String, String> preInfo = ipinfo.getPreMap().get(ips[0]);
//					if(preInfo == null){
//						preInfo = new TreeMap<String, String>();
//						ipinfo.getPreMap().put(ips[0], preInfo);
//					}
//					preInfo.put(ipStart, ipLocalInfo);
				}
